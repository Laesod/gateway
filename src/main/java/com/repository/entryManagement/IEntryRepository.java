...
public interface IEntryRepository extends JpaRepository<EntryEntity, Long> {
    ...
    @Query("select a.entryGuid, a.description, d.entryTypeGuid, f.content from EntryEntity a join a.deficiencyDetails b join b.entryStatus c join a.entryType d join d.translationMap e join e.translations f where a.project.projectGuid=:projectGuid and f.field='name' and f.language=:language order by c.ranking asc, a.createdAt desc")
    ...
    @Query("select a.entryGuid, a.description, d.entryTypeGuid, k.content from EntryEntity a join a.deficiencyDetails b join b.entryStatus c join a.entryType d join a.groups e join a.entryType f join f.translationMap g join g.translations k where a.project.projectGuid=:projectGuid and e.groupGuid IN (:groups) and k.field='name' and k.language=:language order by c.ranking asc, a.createdAt desc")
